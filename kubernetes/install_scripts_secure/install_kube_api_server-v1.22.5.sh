...
[ ! -r /usr/local/bin/kubectl ] && err 1 "${W1_COLOR}${pgm} error: ${N1_COLOR}no such kubectl: ${N2_COLOR}/usr/local/bin/kubectl${N0_COLOR}"
...
KUBERNETES_PUBLIC_ADDRESS="${VIP}"

# TODO: service-account-signing-key-file - personal service-account-key.pem 
...
ExecStart=/usr/local/bin/kube-apiserver \\
  ...
  --service-account-signing-key-file=$CERTIFICATE/certs/server-key.pem \\
