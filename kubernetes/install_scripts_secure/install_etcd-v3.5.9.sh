...
ExecStart=/usr/local/bin/etcd --enable-v2=true --logger=zap
Restart=on-failure
RestartSec=10s
LimitNOFILE=40000
...
#echo "env ETCDCTL_API=3 /opt/etcd/bin/etcdctl --cert $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client.pem --key $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client-key.pem --cacert $CERTIFICATE_MOUNT_PATH/ca.pem endpoint health --endpoints=${ETCDSERVERS}" > /root/etcd-health.sh
 ...
 # for V2:
env ETCDCTL_API=2 /usr/local/bin/etcdctl --cert-file $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client.pem --key-file $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client-key.pem --ca-file $CERTIFICATE_MOUNT_PATH/ca.pem set /coreos.com/network/config '{"Network":"'${FLANNEL_NET}'","Backend": {"Type": "vxlan"}}'
echo "env ETCDCTL_API=2 /usr/local/bin/etcdctl --cert-file $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client.pem --key-file $CERTIFICATE_MOUNT_PATH/$(hostname)-etcd-client-key.pem --ca-file $CERTIFICATE_MOUNT_PATH/ca.pem cluster-health" > /root/etcd-health.sh
